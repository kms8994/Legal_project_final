...
### 17.8 유연한 조문 정규화 적용 결과

- 조문 정규화 규칙을 공통 모듈 `pipelines/common/articles.py`로 분리했다.
- 검색 API, 수집 파이프라인, 구조화 추출 파이프라인, 평가 스크립트가 같은 법령 alias와 normalized ref 생성 규칙을 사용한다.
- 사용자가 입력하는 다음 변형을 같은 조문으로 정규화한다.
  - `민법 제750조`, `민법 제750`, `민법 750`, `민법750조`, `민 법 750` -> `민법_제750조`
  - `자동차손해배상 보장법 제3조`, `자동차손해배상보장법 제3`, `자배법 제3조`, `자동차손배법 3` -> `자동차손해배상 보장법_제3조`
- DB canonical은 현재 저장된 `articles.normalized_ref`와 맞춰 `자동차손해배상 보장법_제3조`를 유지한다.
- `npm.cmd run eval:mvp` 재실행 결과:
  - 조문 검색 전체 `precision@10=0.80`.
  - `missing_scope`를 제외한 in-scope 조문 검색 `precision@10=1.00`.
  - 자연어 검색 `avg_top5_relevant_count=2.0`.
  - 비교 후보 `avg_material_fact_match=0.70`.
  - 비교 후보 `evidence_coverage_rate=1.0`.
  - 구조화 검증 `needs_review_rate=0.77`.
